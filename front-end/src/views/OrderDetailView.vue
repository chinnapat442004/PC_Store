...
<template>
  <div class="max-w-6xl mx-auto p-4 md:p-6 flex flex-col lg:flex-row gap-3">
    <div class="flex-1 flex flex-col gap-3">
      <div
        v-if="orderStore.selectedOrder?.shipment"
      ...
      <div class="bg-white p-4 rounded-xl shadow-sm border">
        ...
        <div class="border-l-2">
          <div
            class="flex items-center gap-4 pl-4 mb-3"
            v-for="(history, index) in orderStore.selectedOrder?.orderHistory"
            :key="history.id"
          >
            <div
    ...
    <div class="w-full lg:w-[350px] flex flex-col gap-4">
      ...
      <div class="bg-white p-4 rounded-xl shadow-sm border">
        ...
        <div class="text-sm text-gray-600">
          ...
          ที่อยู่: {{ orderStore.selectedOrder?.address_detail }}
      ...
      <div class="bg-white p-4 rounded-xl shadow-sm border flex flex-col gap-3">
        <template v-if="orderStore.selectedOrder?.order_status === 'pending'">
          <button
            class="w-full py-2 text-[13px] sm:text-[14px] md:text-[15px] bg-[#637aad] hover:bg-[#4a68a8] text-white rounded-lg transition"
            @click="payOrder(orderStore.selectedOrder.order_id)"
          >
            ชำระเงิน
          </button>
          <button
            class="w-full py-2 text-[13px] sm:text-[14px] md:text-[15px] rounded-lg bg-gray-200 hover:bg-gray-300 border border-gray-400 text-gray-800 transition"
            @click="cancelOrder(orderStore.selectedOrder.order_id)"
          >
            ยกเลิก
          </button>
        </template>
        ...
        <button
          class="w-full py-2 text-[13px] sm:text-[14px] md:text-[15px] border rounded-lg bg-white hover:bg-gray-100 transition"
