...
onMounted(async () => {
  await orderStore.getOrdersByCustomer(orderStore.page, orderStore.limit)
  if (authStore.user) await userStore.getUser(authStore.user?.user_id)
})
...
const saveProfile = async () => {
  try {
    if (userStore.editedProfile) await userStore.updateMyProfile(userStore.editedProfile)
    isEdit.value = false
...
const savePasswordUI = async () => {
  ...
  try {
    ...
    const msg = error?.response?.data?.message || 'รหัสผ่านปัจจุบันไม่ถูกต้อง'
    alert(msg)
  }
}
</script>
    ...
    <div class="max-w-[900px] w-full flex flex-col gap-2 md:gap-4">
      ...
      <div class="p-4 bg-white rounded-[12px] shadow-sm">
        ...
        <div v-else-if="userStore.editedProfile" class="flex flex-col gap-3">
          ...
          <input
            v-model="userStore.editedProfile.name"
            type="text"
            placeholder="ชื่อ"
          ...
          <input
            v-model="userStore.editedProfile.email"
            type="email"
            placeholder="Email"
            disabled
          ...
          <div class="flex gap-2 justify-end">
            <button @click="cancelEdit" class="px-3 py-1 text-sm border rounded">ยกเลิก</button>
      ...
      <div class="p-4 bg-white rounded-[12px] shadow-sm">
        ...
        <div v-else class="flex flex-col gap-3 mt-3">
          ...
          <input
            v-model="userStore.updatePasswordForm.current_password"
            type="password"
          ...
          <input
            v-model="userStore.updatePasswordForm.new_password"
            type="password"
          ...
          <input
            v-model="userStore.updatePasswordForm.confirm_password"
            type="password"
