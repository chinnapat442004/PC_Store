...
const availableSubDistricts = ref<ThaiAddress[]>([])
const selectedSubDistrict = ref<ThaiAddress | null>(null)
const thaiAddressDB = ref<ThaiAddress[]>([])
...
const editAddress = (addr: Address) => {
  ...
  if (addr.zipcode) {
    ...
    selectedSubDistrict.value = matches.find((item) => item.district === addr.sub_district) || null
  }
...
const paymentMethods = [
  { label: 'QR พร้อมเพย์', value: 'promptpay' },
...
const useCode = handleCouponSubmit(async (values) => {
  try {
    ...
    setFieldError('code', err?.response?.data?.message || 'โค้ดไม่ถูกต้อง')
  }
})
    ...
    <div class="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-3">
      <div class="bg-white p-6 rounded-lg shadow-md">
          ...
          >
            <div>
              ...
              <div class="text-sm text-gray-600">
                ...
                ต.{{ defaultAddress.sub_district }} อ.{{ defaultAddress.district }} จ.{{
                  defaultAddress.province
                }}
                {{ defaultAddress.zipcode }}
              </div>
              <div class="text-sm text-blue-500">เปลี่ยน</div>
            </div>
          </div>
        ...
        <div v-else>
          <div class="flex justify-between items-center mb-4">
            ...
            <button class="text-blue-500" @click="showAddressManager = false">กลับ</button>
          </div>
          ...
          <div class="space-y-3 mb-4">
            <div
              ...
              :key="addr.address_id"
              class="border p-3 rounded-lg flex gap-3 items-start"
              :class="
            ...
            >
              <div class="flex-1">
                ...
                <div class="text-sm text-gray-600">
                  {{ addr.address_detail }}
                  ต.{{ addr.sub_district }} อ.{{ addr.district }} จ.{{ addr.province }}
                  {{ addr.zipcode }}
                </div>
                ...
                <div class="flex justify-between mt-2 text-sm">
                  <button
                    ...
                    class="px-3 py-1 text-white rounded bg-[#82d182] hover:bg-[#69c769]"
                  ...
                  <div class="flex gap-2">
                    <button class="text-blue-500" @click.stop="editAddress(addr)">แก้ไข</button>
                    <button class="text-red-500" @click.stop="removeAddress(addr)">ลบ</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div class="bg-white p-6 rounded-lg shadow-md h-fit">
          ...
          <div v-if="!cartStore.cart?.coupon" class="flex flex-col gap-2">
            <div class="flex gap-2">
              ...
              <button
                ...
                class="text-white bg-[#637aad] hover:bg-[#4a68a8] rounded-lg px-3 py-1 disabled:bg-gray-400 disabled:text-gray-200 disabled:cursor-not-allowed disabled:hover:bg-gray-400"
                @click="useCode"
              >
                ใช้โค้ด
              </button>
        ...
        <button
          ...
          @click="goToOrderSuccess(cartStore.cart?.coupon?.code)"
        >
          ยืนยันคำสั่งซื้อ
        </button>
      </div>
  ...
  <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div class="relative w-full max-w-md max-h-[90vh] z-10">
      <div class="bg-white p-5 rounded-xl shadow-lg overflow-y-auto max-h-[90vh]">
        ...
        <input
          v-model="addressStore.editedAddress.fullname"
          placeholder="ชื่อ-นามสกุล"
          class="border w-full mb-2 p-2 rounded bg-gray-50"
        />
        <input
          v-model="addressStore.editedAddress.phone"
          placeholder="เบอร์โทรศัพท์"
          class="border w-full mb-2 p-2 rounded bg-gray-50"
          maxlength="10"
        />
        <textarea
          v-model="addressStore.editedAddress.address_detail"
        ...
        <div class="grid grid-cols-2 gap-2 mb-2">
          <div>
            <input
              v-model="addressStore.editedAddress.zipcode"
              placeholder="รหัสไปรษณีย์"
              class="border w-full p-2 rounded bg-gray-50"
              maxlength="5"
              @input="findAddressByZipcode"
            />
          </div>
          <div>
            <Listbox v-model="selectedSubDistrict">
              <div class="relative">
                <ListboxButton
                  :disabled="!availableSubDistricts.length"
                ...
                >
                  ...
                  >
                    <ListboxOption
                      v-for="(item, index) in availableSubDistricts"
                      :key="index"
                      :value="item"
        ...
        <div class="grid grid-cols-2 gap-2 mb-4">
          <input
            v-model="addressStore.editedAddress.district"
            placeholder="อำเภอ/เขต"
            class="border w-full p-2 bg-gray-200 text-gray-600 rounded"
            readonly
          />
          <input
            v-model="addressStore.editedAddress.province"
            placeholder="จังหวัด"
            class="border w-full p-2 bg-gray-200 text-gray-600 rounded"
            readonly
          />
        ...
        <div class="flex gap-2 justify-end">
          <button class="bg-gray-300 px-4 py-2 rounded" @click="showForm = false">ยกเลิก</button>
          <button
            class="bg-[#82d182] hover:bg-[#69c769] text-white px-4 py-2 rounded"
            @click="handleSave"
          >
            บันทึก
          </button>
