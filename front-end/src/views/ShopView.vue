...
<template>
  <div class="w-full h-full lg:px-[20px]">
    <div
      class="flex flex-col lg:flex-row justify-center py-[30px] px-[10px] md:px-[20px] max-w-[1300px] m-auto gap-4"
    >
      <div class="flex flex-col gap-3 items-center">
        ...
        >
          ...
          <div class="flex flex-wrap lg:flex-col gap-3 lg:gap-1">
            <div
              class="p-[5px] flex items-center"
              v-for="item of categoryStore.categories.filter((c) => c.is_active)"
            ...
            >
              ...
              <div>
                <label
                  :for="`checkbox-${item.category_id}`"
                  class="cursor-pointer text-[15px] md:text-[17px]"
                >
                  {{ item.name }}
                </label>
              </div>
      ...
      <div class="flex-1 w-full">
        ...
        >
          <div class="flex justify-center" v-for="item of paginatedProducts" :key="item.product_id">
            ...
            >
              <div
                class="relative flex justify-center flex-1 overflow-hidden rounded-[5px] bg-gray-50"
              >
                <img
                  ...
                  :src="
                    item.images && item.images.length > 0
                      ? item.images[0].image.replace('w_400', 'w_200')
                      : ''
                  "
                  alt=""
                  loading="lazy"
              ...
              <div class="flex justify-between flex-col h-[70px] md:h-[90px] pt-[10px]">
                <div
                  class="font-bold text-sm md:text-base line-clamp-2 md:line-clamp-1 break-words leading-tight"
                  :class="{ 'text-gray-400': item.stock_quantity === 0 }"
                  :title="item.title"
                >
                  {{ item.title }}
                </div>
                ...
                <div class="flex justify-between items-end mt-1">
                  ...
                  <div class="text-[10px] md:text-base font-medium">
                    ...
                    <span
                      v-else-if="item.stock_quantity <= 5"
